...
export const processNewPayment = async (paymentData) => {
  ...
  if (normalizedStatus !== "SUCCESS") {
    logger.log(
      `Skipping transaction ${transactionId} with status ${status}`,
      "Worker task"
    );
    return false;
  }
  ...
  try {
    ...
    if (processed) {
      logger.log(
        `[IDEMPOTENCY_BLOCK] Transaction ${transactionId} already processed.`,
        "Worker task"
      );
    ...
    logger.log(
      `Transaction ${transactionId} successfully recorded and balance updated`,
      "Worker task"
    );
    return true;
  } catch (error) {
